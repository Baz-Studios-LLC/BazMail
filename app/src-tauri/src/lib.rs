...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // The macOS title bar is configured in tauri.macos.conf.json, not
            // here. Turning decorations on after the fact left the native
            // traffic lights drawn but dead — they need to be part of how the
            // window is created, not something bolted on once it exists. The
            // platform config file is merged at build time, so the window is
            // born with the right style and this code stays platform-free.
